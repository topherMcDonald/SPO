...
void SetupTab::on_btnAddLineItem_GetMacPacPart_clicked()
{
    ...
    if(dialog->exec())
    {
        ...
        //replaced with spinBox ui->leAddLineItem_Quantity->setText("1");
...
void SetupTab::updateExtendedCostTotal()
{
    ...
    for (int i = 0; i < rowCt; i++) {
        ...
        tmpTotal += wi->text().toDouble();
        qDebug() << QString("print the tmoptotasl value") << QString::number(tmpTotal);
...
void SetupTab::on_btnAddLineItem_AddLine_clicked()
{
    ...
    //ChrisMcD: Repalced^^^^^^^ // START: Extended Cost Mod - FJD - 8.9.20
...
void SetupTab::handleDeleteSelectedRow()
{
    ...
    while(selectionRangeListIter.hasNext()){
       ...
       /// QModelIndex currentIndex = rowIndex;
       if(columnIndex == 0)
        {
            qDebug() << this->ui->tblOrderLinesWidget->item(rowIndex,2)->text();
            columnIndex++;
        }
...
void SetupTab::WriteXml()
{
    ...
    //////////////////////////////////////////////// uncomment for delivery, live         PostXMLToService(xmlToServiceAry);
}
/************************************************************************
 *
 *  PostXMLToService()
